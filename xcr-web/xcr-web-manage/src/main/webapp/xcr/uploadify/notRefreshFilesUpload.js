/**
 * Created by huaxin on 2017/4/28.
 */

/*
 * 无刷新异步上传插件 2013-10-16 Devotion Created
 */
(function($) {
	var defaultSettings = {
		url : "", // 上传地址
		buttonFeature : true, // true:点击按钮时仅选择文件； false:选择完文件后立即上传
		fileSuffixs : ["jpg", "png", "bmp"], // 允许上传的文件后缀名列表
		...
		onCheckUpload : function(text) { // 上传时检查文件后缀名不包含在fileSuffixs属性中时触发的回调函数，(text为错误提示文本)
			...
			console.log("asdasdasdaaaaaaaaaaaa");
		...
		onChosen : function(file, obj) { // 选择文件后的回调函数，(file为选中文件的本地路径;obj为当前的上传控件实例)
			if (file) {
				...
				var gr = file.substring(index + 1).toLowerCase();
				if ($.inArray(gr, this.fileSuffixs)) {
					alert("不符合文件格式（" + this.fileSuffixs.join(',') + "）");
					return false;
				}
				return true;
			}
		...
		perviewImgStyle : null,// 用于设置图片预览时的样式（可不设置，在不设置的情况下多文件上传时只能显示一张图片），如{
		// width: '100px', height: '100px', border: '1px
		// solid #ebebeb' }

		singleImgSize : 0
	// 限制单个文件上传大小，单位是bytes，如果是1M的，则数值为1024*1024
	};
...
UploadAssist.prototype = {
	...
	createIframe : function(/* 插件中指定的dom对象 */elem) {
		...
		var html = "<html>"
				...
				+ "</head>"
				+ "<body>"
				+ "<form method='post' target='dynamic_creation_upload_iframe' enctype='multipart/form-data' action='"
	...
	createInputFile : function() {
		...
		// input.setAttribute("time", new Date().getTime());
		...
		input.onchange = function() {
			...
			// 选中后的回调
			if (!that.settings.onChosen(this.value, this)) {
				return false;
			}
...
var perviewImage = {
	...
	createPreviewElement : function(/* 上传时的文件名 */file, /* 预览时的样式 */style) {
		...
		close.setAttribute("class", "close_img_by_setting");
		...
		// main.appendChild(text); //上传图片的名称
		main.appendChild(close);
		return main;
	},

	// 设置不显示删除图片按钮
	isCloseImg : function(isclose) {
		var close = document.querySelectorAll('div img.close_img_by_setting');
		var len = close.length;
		for (var i = 0; i < len; i++) {
			close[i].style.visibility = isclose ? "hidden" : "visible";
		}
	},
