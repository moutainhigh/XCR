...
var actionFormatter = function (value, row, index) {//赋予的参数
    console.log("row.settleNo:" + row.settleNo);
    ///toc/settle/detail/{id}/List.htm
    var detailUrl = "/xcr/toc/settleDetail/" + row.settleNo + "/" + 1 + "/List.htm";
    return [
        '<a href="javascript:loadHtml(\'' + detailUrl + '\')" class="btn"><h4><i class="fa fa-search-plus"></i>详情</h4></a>'
    ].join('');
};

$('#loginLog-table').bootstrapTable({
    url: "/xcr/toc/usualSettle/List.htm",
