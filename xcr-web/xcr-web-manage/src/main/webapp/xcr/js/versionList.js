...
$('#loginLog-table').bootstrapTable({
    ...
    columns: columns = [
        ...
        , {
            ...
            title: '版本说明',
            align: 'center',
            formatter: versionDescriptionFormatter,
            valign: 'middle',
            sortable: false,
            width:350
        }, {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (state == 1) {
                ...
                $("input[name=isLiveUp]").attr("disabled",true);
            }
...
function recommendCode(obj){
	var type = $("#type").val();
	if(type==2){//供应链
		var name = $(obj).val();
		$.ajax({
	        url: "/xcr/RecommendMaxCode.htm",
	        data: {versionCode: name,type:type},
	        type: "post",
	        success: function (data) {
	        	$("#code").val(data.resultObject);
	        }
	    });
	}else{//IOS以及安卓
		$.ajax({
